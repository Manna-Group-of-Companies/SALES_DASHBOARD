...
async function listNotifications(user: User): Promise<AppNotification[]> {
  ...
  // `Sales Notification` is not on the site yet — verified 11 Aug 2026, it
  // answers 404. An absent feed is an empty bell, not a dead screen: the write
  // side twelve lines up is fire-and-forget for the same reason.
  const rows = await listDocs<AppNotification>(DOCTYPE.notification, {
    filters: [['audience', 'like', `%${user.role}%`]],
    orderBy: 'creation desc',
    limit: 200,
  }).catch(ifMissing<AppNotification[]>([], DOCTYPE.notification));
